...
const fetchPost = async () => {
  const res = await axios.get(
    ...
    `${import.meta.env.VITE_API_URL}/posts?featured=true&limit=8&sort=newest`
  );
  return res.data;
};
  ...
  return (
    ...
    <div className='mt-8 flex flex-col lg:flex-row gap-8' >
      ...
      <div className="w-full lg:w-1/2 flex flex-col gap-4">
        ...
        {posts[2] && <div className="lg:h-1/3 flex justify-between gap-4 py-2 border-b border-[rgb(230,232,234)]">
          {/* <img src="img_04.jpg" alt="" className="rounded-sm object-cover w-1/3"
            w="298" /> */}
          {posts[7].img && <div className="w-1/3 aspect-video">
            <Image
              src={posts[7].img}
          ...
          <div className="w-2/3 ">
            ...
            <div className="flex items-center gap-4 text-sm lg:text-base mt-4">
              ...
              <Link className="text-blue-800">{posts[7].category}</Link>
              <span className="text-gray-500 text-sm">{format(posts[7].createdAt)}</span>
            </div>
          </div>
        </div>}
        {/* fourth */}
